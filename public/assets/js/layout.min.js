var Layout=function() {
  const e='layouts/layout2/img/'; const a='layouts/layout2/css/'; const i=App.getResponsiveBreakpoint('md'); const t=[]; const s=[]; const o=function() {
    let e; const a=$('.page-content'); const t=$('.page-sidebar'); const s=$('body'); if (s.hasClass('page-footer-fixed')===!0&&s.hasClass('page-sidebar-fixed')===!1) {
      const o=App.getViewPort().height-$('.page-footer').outerHeight()-$('.page-header').outerHeight(); a.height()<o&&a.css('min-height', o);
    } else {
      if (s.hasClass('page-sidebar-fixed'))e=d(), s.hasClass('page-footer-fixed')===!1&&(e-=$('.page-footer').outerHeight()); else {
        const n=$('.page-header').outerHeight(); const r=$('.page-footer').outerHeight(); e=App.getViewPort().width<i?App.getViewPort().height-n-r:t.height()+20, e+n+r<=App.getViewPort().height&&(e=App.getViewPort().height-n-r);
      }a.css('min-height', e);
    }
  }; const n=function(e, a, t) {
    const s=location.hash.toLowerCase(); const o=$('.page-sidebar-menu'); if ('click'===e||'set'===e?a=$(a):'match'===e&&o.find('li > a').each(function() {
      const e=$(this).attr('ui-sref'); if (t&&e) {
        if (t.is(e)) return void(a=$(this));
      } else {
        let i=$(this).attr('href'); if (i&&(i=i.toLowerCase(), i.length>1&&s.substr(1, i.length-1)==i.substr(1))) return void(a=$(this));
      }
      ...
      parseInt(o.data('slide-speed')), o.data('keep-expanded'); o.hasClass('page-sidebar-menu-hover-submenu')===!1?o.find('li.nav-item.open').each(function() {
        let e=!1; $(this).find('li').each(function() {
          const i=$(this).attr('ui-sref'); if (t&&i) {
    ...
    $('.page-sidebar-menu').on('click', 'li > a.nav-toggle, li > a > span.nav-toggle', function(e) {
      const a=$(this).closest('.nav-item').children('.nav-link'); if (!(App.getViewPort().width>=i&&!$('.page-sidebar-menu').attr('data-initialized')&&$('body').hasClass('page-sidebar-closed')&&1===a.parent('li').parent('.page-sidebar-menu').size())) {
        const t=a.next().hasClass('sub-menu'); if (!(App.getViewPort().width>=i&&1===a.parents('.page-sidebar-menu-hover-submenu').size())) {
          if (t===!1) return void(App.getViewPort().width<i&&$('.page-sidebar').hasClass('in')&&$('.page-header .responsive-toggler').click()); const s=a.parent().parent(); const n=a; const r=$('.page-sidebar-menu'); const l=a.next(); const d=r.data('auto-scroll'); const p=parseInt(r.data('slide-speed')); const c=r.data('keep-expanded'); c!==!0&&(s.children('li.open').children('a').children('.arrow').removeClass('open'), s.children('li.open').children('.sub-menu:not(.always-open)').slideUp(p), s.children('li.open').removeClass('open')); const h=-200; l.is(':visible')?($('.arrow', n).removeClass('open'), n.parent().removeClass('open'), l.slideUp(p, function() {
      ...
      e.preventDefault(), App.scrollTop(); const a=$(this).attr('href'); const t=$('.page-sidebar ul'); t.children('li.active').removeClass('active'), t.children('arrow.open').removeClass('open'), $(this).parents('li').each(function() {
      ...
      e.preventDefault(), App.scrollTop(); const a=$(this).attr('href'); App.startPageLoading(), App.getViewPort().width<i&&$('.page-sidebar').hasClass('in')&&$('.page-header .responsive-toggler').click(), Layout.loadAjaxContent(a);
    ...
    let e=App.getViewPort().height-$('.page-header').outerHeight(); return $('body').hasClass('page-footer-fixed')&&(e-=$('.page-footer').outerHeight()), e;
  }; const p=function() {
    const e=$('.page-sidebar-menu'); return App.destroySlimScroll(e), 0===$('.page-sidebar-fixed').size()?void o():void(App.getViewPort().width>=i&&(e.attr('data-height', d()), App.initSlimScroll(e), o()));
  }; var c=function() {
    const e=$('body'); e.hasClass('page-sidebar-fixed')&&$('.page-sidebar').on('mouseenter', function() {
    ...
    const e=300; const a=500; navigator.userAgent.match(/iPhone|iPad|iPod/i)?$(window).bind('touchend touchcancel touchleave', function(i) {
...
$(this).scrollTop()>e?$('.scroll-to-top').fadeIn(a):$('.scroll-to-top').fadeOut(a);
    ...
    $('.full-height-content').each(function() {
      let e; const a=$(this); if (e=App.getViewPort().height-$('.page-header').outerHeight(!0)-$('.page-footer').outerHeight(!0)-$('.page-title').outerHeight(!0)-$('.page-bar').outerHeight(!0), a.hasClass('portlet')) {
        const t=a.find('.portlet-body'); App.destroySlimScroll(t.find('.full-height-content-body')), e=e-a.find('.portlet-title').outerHeight(!0)-parseInt(a.find('.portlet-body').css('padding-top'))-parseInt(a.find('.portlet-body').css('padding-bottom'))-2, App.getViewPort().width>=i&&a.hasClass('full-height-content-scrollable')?(e-=35, t.find('.full-height-content-body').css('height', e), App.initSlimScroll(t.find('.full-height-content-body'))):t.css('min-height', e);
    ...
    const i=$('.page-content .page-content-body'); App.startPageLoading({animate: !0}), $.ajax({type: 'GET', cache: !1, url: e, dataType: 'html', success: function(e) {
      App.stopPageLoading(); for (let s=0; s<t.length; s++)t[s].call(e); a.size()>0&&0===a.parents('li.open').size()&&$('.page-sidebar-menu > li.open > a').click(), i.html(e), Layout.fixContentHeight(), App.initAjax();
    }, error: function(e, a, o) {
      App.stopPageLoading(), i.html('<h4>Could not load the requested content.</h4>'); for (let n=0; n<s.length; n++)t[n].call(e);
    }});
